...
// Gerenciador da galeria de fotos da página "Espaços".
// Acrescentar e remover fotos, editar legenda — salva direto no banco
// (seção "galeria" de page_content) e o site reflete na hora.
export default function GaleriaEspacos() {
  const [toast, toastNode] = useToast();
  const { sec, loading } = usePageContent("espacos");
  const [fotos, setFotos] = useState<GalFoto[]>(ESPACOS_GAL_DEFAULT);
  const [subindo, setSubindo] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (!loading) setFotos(section<GalFoto[]>(sec, "galeria", ESPACOS_GAL_DEFAULT));
  }, [loading]); // eslint-disable-line react-hooks/exhaustive-deps

  const persist = async (next: GalFoto[]) => {
    setFotos(next);
    if (!API_CONFIGURED) return;
    try { await savePage("espacos", { galeria: next }); }
  ...
  const enviar = async (files: FileList) => {
    ...
    for (const file of Array.from(files)) {
      try {
        const url = await uploadImagem(file, "espacos");
  ...
  const remover = async (i: number) => {
    const f = fotos[i];
    if (!window.confirm(`Remover esta foto${f.titulo ? ` ("${f.titulo}")` : ""} da galeria?`)) return;
  ...
  const salvarLegendas = async () => { await persist(fotos); toast("Legendas salvas!"); };

  return (
    <div className="adm-card" style={{ marginTop: 18 }}>
      <div className="adm-card-sec"><div className="si"><i className="fa-regular fa-images"></i></div><h3>Galeria de fotos ({fotos.length})</h3></div>
      <p className="hint" style={{ marginBottom: 12 }}>Adicione ou remova as fotos que aparecem na galeria da página Espaços. A legenda aparece ao ampliar a foto.</p>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
        ...
        <button className="adm-btn adm-btn-ghost adm-btn-sm" style={{ width: "auto" }} onClick={salvarLegendas}><i className="fa-solid fa-floppy-disk"></i> Salvar legendas</button>
      </div>
          ...
          {fotos.map((f, i) => (
            <div key={i} style={{ border: "1px solid var(--adm-line)", borderRadius: 12, overflow: "hidden", background: "#fff" }}>
              ...
              <div style={{ padding: "8px 10px" }}>
                <input className="adm-text" value={f.titulo} placeholder="Título (ex.: Biblioteca)" onChange={(e) => editar(i, "titulo", e.target.value)} onBlur={salvarLegendas} style={{ padding: "6px 8px", fontSize: 12.5, marginBottom: 6 }} />
                <input className="adm-text" value={f.descricao} placeholder="Legenda" onChange={(e) => editar(i, "descricao", e.target.value)} onBlur={salvarLegendas} style={{ padding: "6px 8px", fontSize: 12.5 }} />
              </div>
            </div>
          ))}
        </div>
